<x-app-layout>
	...
	<section class="bg-stone-200 mx-auto w-5/6  py-12">
		@if(session('alert'))
		...
		@endif
		<form method="post" action="{{ route('expenses-type-create')}}" class="w-96 mx-auto  space-y-3">
			@csrf
			 <div>
				<label for="type" class="block font-semibold text-md mb-2">Type</label>
				<input id="type" type="text" name="type" value="{{old('type')}}" class="w-full" placeholder="Type">
				@error('type')
					<p class="text-sm">{{$message}}</p>
				@enderror
			 </div>
			 <button type="" name="save" class="w-full bg-rose-500 px-3 py-px rounded shadow-lg shadow-rose-500/70">Save</button>
		</form>
	</section>
	<section>
						...
						<td class="px-6 py-1.5">
							...
							<div class="flex items-center justify-center gap-1">
								...
								<form method="post" action="{{route('expenses-type-visibility')}}">
									@csrf
									<x-button name="visibilityId" value="{{$type->id}}" class="justify-center h-7 w-6 {{$type->visibility ==1 ? 'bg-blue-500 shadow-blue-500/70' : 'bg-gray-700 shadow-gray-700/70'}} text-white shadow-lg">
									...
									<x-button :href="route('type-update',['code'=>$type->code])" class="justify-center bg-yellow-500 shadow-yellow-500/70 text-white shadow-lg h-7 w-6">
										<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-4 h-4">
								...
								<form method="post" action="{{route('expenses-type-delete')}}">
									@csrf
									<x-button name="deleteId" value="{{$type->id}}" class="justify-center bg-rose-500 shadow-rose-500/70 text-white shadow-lg h-7 w-6">
										<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-4 h-4">
