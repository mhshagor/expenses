<div>
			...
			<table class="w-full text-sm text-left text-gray-500 dark:text-gray-400 rounded-lg">
				...
				<tbody class="bg-gray-200 border-b border-gray-400">
					...
					<tr class="bg-blue-100 border-b hover:bg-gray-50 text-center text-black {{$data->visibility != 1 ? 'opacity-25 hover:opacity-75' : ''}}">
						...
						<td class="px-6 py-1.5">{{$data->code}}</td>
						<td class="px-6 py-1.5">{{$data->product_name}}</td>
						<td class="px-6 py-1.5">{{$data->product_details}}</td>
						<td class="px-6 py-1.5">{{$data->product_price}}</td>
						<td class="px-6 py-1.5">{{$data->product_quantity}}</td>
						<td class="px-6 py-1.5">
							<img class="w-16 mx-auto h-9 border-4" src="{{env('APP_URL').'/'.(($data->product_image) ?? 'public/images/demo-image.bmp')}}">
						</td>
	...
	<x-jet-dialog-modal wire:model="dataAddModalOpen">
		...
		<x-slot name="content">
			...
			<div>
				...
				<select wire:model="addSubcategory" class="w-full py-1 rounded">
					@if($addCategory)
			...
			<div class="relative">
				...
				<input id="addName" type="text" wire:model="addName" class="w-full py-1 rounded" placeholder="Product Name">
				@error('addName')
			...
			<!-- product_details -->
			<div class="relative">
				<label for="addDetails" class="block font-semibold text-md">Details</label>
				<textarea id="addDetails" wire:model="addDetails" class="w-full py-1 rounded" placeholder="Product Details"></textarea>
				@error('addDetails')
				<p class="text-xs font-semibold text-rose-600 absolute inset-0 top-auto -bottom-3.5 mr-1 ">{{$message}}</p>
				@enderror
			</div>
			<!-- product_price -->
			<div class="relative">
				<label for="addPrice" class="block font-semibold text-md">Price</label>
				<input id="addPrice" type="number" wire:model="addPrice" class="w-full py-1 rounded" placeholder="Product Price">
				@error('addPrice')
				<p class="text-xs font-semibold text-rose-600 absolute inset-0 top-auto -bottom-3.5 mr-1 ">{{$message}}</p>
				@enderror
			</div>
			<!-- product_quantity -->
			<div class="relative">
				<label for="addQuantity" class="block font-semibold text-md">Quantity</label>
				<input id="addQuantity" type="number" wire:model="addQuantity" class="w-full py-1 rounded" placeholder="Product Quantity">
				@error('addQuantity')
				<p class="text-xs font-semibold text-rose-600 absolute inset-0 top-auto -bottom-3.5 mr-1 ">{{$message}}</p>
				@enderror
			</div>
	...
	<x-jet-dialog-modal wire:model="dataEditModalOpen">
		<x-slot name="title">
			Edit  <span class="text-yellow-400">{{$targetedData->product_name ?? ''}}</span>
		</x-slot>
		<x-slot name="content">
			<!-- Category -->
			<div>
			...
			<div>
				<p class="font-semibold text-md">Select Subcategory</p>
				{{--$test--}}
				<select wire:model="editSubcategory" class="w-full py-1 rounded">
					@if($editCategory)
						@forelse($subcategories as $subcategory)
							@if($loop->first)
								<option value="">Select Subcategory</option>
			...
			<div class="relative">
				...
				<input id="editName" type="text" wire:model="editName" class="w-full py-1 rounded" placeholder="Product Name">
				@error('editName')
				<p class="text-xs font-semibold text-rose-600 absolute inset-0 top-auto -bottom-3.5 mr-1 ">{{$message}}</p>
				@enderror
			</div>
			<!-- product_details -->
			<div class="relative">
				<label for="editDetails" class="block font-semibold text-md">Details</label>
				<textarea id="editDetails" wire:model="editDetails" class="w-full py-1 rounded" placeholder="Product Details"></textarea>
				@error('editDetails')
				<p class="text-xs font-semibold text-rose-600 absolute inset-0 top-auto -bottom-3.5 mr-1 ">{{$message}}</p>
				@enderror
			</div>
			<!-- product_price -->
			<div class="relative">
				<label for="editPrice" class="block font-semibold text-md">Price</label>
				<input id="editPrice" type="number" wire:model="editPrice" class="w-full py-1 rounded" placeholder="Product Price">
				@error('editPrice')
				<p class="text-xs font-semibold text-rose-600 absolute inset-0 top-auto -bottom-3.5 mr-1 ">{{$message}}</p>
				@enderror
			</div>
			<!-- product_quantity -->
			<div class="relative">
				<label for="editQuantity" class="block font-semibold text-md">Quantity</label>
				<input id="editQuantity" type="number" wire:model="editQuantity" class="w-full py-1 rounded" placeholder="Product Quantity">
				@error('editQuantity')
				<p class="text-xs font-semibold text-rose-600 absolute inset-0 top-auto -bottom-3.5 mr-1 ">{{$message}}</p>
				@enderror
			</div>
