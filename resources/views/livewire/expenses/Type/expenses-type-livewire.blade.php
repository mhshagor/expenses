				...
				<tbody class="bg-gray-200 border-b border-gray-400">
				  @foreach($expensesTypes as $data)
					<tr class="bg-blue-100 border-b hover:bg-gray-50 text-center text-black {{$data->visibility != 1 ? 'opacity-25 hover:opacity-75' : ''}}">
						...
						<td class="px-6 py-1.5">{{ucwords($data->policy)}}</td>
						<td class="px-6 py-1.5">{{$data->name}}</td>
						...
						<td class="px-6 py-1.5">
							<div class="flex items-center justify-center gap-1">
								...
								<!-- Edit -->
								<x-button wire:click="dataEditAction({{$data->code}})" class="justify-center bg-yellow-500 hover:shadow-yellow-500/70 text-white hover:shadow-lg duration-500 h-7 w-6  {{$data->visibility ==1 ? '' : 'hidden'}}">
									<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-4 h-4">
