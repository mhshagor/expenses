<x-app-layout>
	...
	<section class="mx-auto w-5/6  py-12">
		<form method="post" action="{{ route('business-category-create')}}" class="w-96 mx-auto space-y-3 p-10 border border-4 shadow-lg shadow-stone-500 border-stone-50" enctype="multipart/form-data">
			@csrf
			<div>
				<label for="category_name" class="block font-semibold text-md mb-2">Category Name:</label>
				<input id="type" type="text" name="category_name" value="{{old('category_name')}}" class="w-full" placeholder="Type Category Name">
				@error('category_name')
	...
	<section>
		<div class="mx-auto w-5/6 border border-4 border-stone-50 shadow-lg shadow-stone-500 relative overflow-x-auto shadow-md sm:rounded-lg overflow-hidden">
					...
					<tr class="bg-blue-100 border-b hover:bg-gray-50 text-center text-black {{$category->visibility != 1 ? 'opacity-25 hover:opacity-75' : ''}}">
						...
						<td class="px-6 py-1.5">
							<div class="flex items-center justify-center gap-1">
								<!-- Visibility -->
								<form method="post" action="{{route('category-visibility')}}">
									@csrf
									<x-button name="visibilityId" value="{{$category->id}}" class="justify-center h-7 w-6 {{$category->visibility ==1 ? 'bg-blue-500 hover:shadow-blue-500/70' : 'bg-gray-700 hover:shadow-gray-700/70'}} text-white hover:shadow-lg duration-500">
								...
								<!-- Delete -->
								<form method="post" action="{{route('category-delete')}}"class=" {{$category->visibility != 1 ? 'hidden' : ''}}">
									@csrf
									<x-button name="deleteId" value="{{$category->id}}" class="justify-center bg-rose-500 hover:shadow-rose-500/70 text-white hover:shadow-lg duration-500 h-7 w-6">
										<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-4 h-4">
			...
			{{ $BusinessCategory->links('components.custom_pagination') }}
		</div>
	</section>
